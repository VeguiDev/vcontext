...
import { VCONTEXT_HOME } from "./index.js";
...
export class IdentityStore {
  ...
  get(origin: string): LocalIdentity | null {
    return this.read().origins[new URL(origin).origin] ?? null;
  }

  set(
    origin: string,
    identity: Omit<LocalIdentity, "updated_at">,
  ): LocalIdentity {
    const normalizedOrigin = new URL(origin).origin;
    const current = this.read();
    current.origins[normalizedOrigin] = {
      ...identity,
    ...
    return current.origins[normalizedOrigin]!;
  }
  ...
  private read(): IdentityFile {
    try {
      ...
      return value.version === 1 && value.origins
        ? value
        : { version: 1, origins: {} };
    } catch {
